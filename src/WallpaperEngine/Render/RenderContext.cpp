...
RenderContext::RenderContext (Context& context) :
    m_textureCache (*context.texture_cache), m_context(context) { }
...
void RenderContext::setWallpaper (std::unique_ptr<CWallpaper> wallpaper) {
    this->m_wallpaper = std::move (wallpaper);
}
...
const CWallpaper& RenderContext::getWallpaper () const {
